...
function monemploi_ajax_scripts() {
	...
	wp_register_script( 'monemploi-ajax-add-job-scripts', $url . "js/ajax.monemploi.add.job.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-add-job-scripts', 'add_job_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-add-job-scripts' );
	...
	wp_register_script( 'monemploi-ajax-remove-attachment-scripts', $url . "js/ajax.monemploi.remove.attachment.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-remove-attachment-scripts', 'remove_attachment_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-remove-attachment-scripts' );
	
	wp_register_script( 'monemploi-ajax-comment-candidacy-scripts', $url . "js/ajax.monemploi.comment.candidacy.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-comment-candidacy-scripts', 'comment_candidacy_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-comment-candidacy-scripts' );
			
	wp_register_script( 'monemploi-ajax-delete-comment-candidacy-scripts', $url . "js/ajax.monemploi.delete.comment.candidacy.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-delete-comment-candidacy-scripts', 'delete_comment_candidacy_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-delete-comment-candidacy-scripts' );
	
	wp_register_script( 'monemploi-ajax-avis-employeur-scripts', $url . "js/ajax.monemploi.avis.employeur.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-avis-employeur-scripts', 'avis_employeur_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-avis-employeur-scripts' );
	
	wp_register_script( 'monemploi-ajax-avis-employeur-delete-scripts', $url . "js/ajax.monemploi.delete.avis.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-avis-employeur-delete-scripts', 'delete_avis_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-avis-employeur-delete-scripts' );
	
	wp_register_script( 'monemploi-ajax-avis-employer-scripts', $url . "js/ajax.monemploi.avis.employer.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-avis-employer-scripts', 'avis_employer_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-avis-employer-scripts' );
	
	wp_register_script( 'monemploi-ajax-avis-employer-delete-scripts', $url . "js/ajax.monemploi.delete.avis.employer.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-avis-employer-delete-scripts', 'delete_avis_employer_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-avis-employer-delete-scripts' );
...
/* AJAX action callback */
add_action( 'wp_ajax_monemploi_add_job', 'monemploi_add_job' );
add_action( 'wp_ajax_nopriv_monemploi_add_job', 'monemploi_add_job' );
function monemploi_add_job($post) {
...
add_action( 'wp_ajax_nopriv_remove_attachment', 'remove_attachment' );
function remove_attachment($post) {
	...
	if ( $deleted_attachment ) {
		wp_send_json("Attachment ID '.$object_id.' permanently deleted.");
	} else {
		wp_send_json("Error deleting attachment ID '.$object_id.' .");
	}
...
/* AJAX action callback */
add_action( 'wp_ajax_comment_candidacy_save', 'comment_candidacy_save' );
add_action( 'wp_ajax_nopriv_comment_candidacy_save', 'comment_candidacy_save' );
function comment_candidacy_save($post) {
	...
	$get_comment_by_id = wpse120039_get_comment_by_id($comment_id);
	...
	$html[] = '<div class="candidacy-response-cards-wrapper ' . $ramdonstring . '">';
		$html[] .= '<div class="ns-row">';
			...
			$html[] .= '<div class="ns-col-sm-3 response-dates">';
				$html[] .= '<a href="#response-' . esc_attr($numItems) . '" class="response-bookmark ns-small">' . date( 'd M Y h:iA', strtotime( $response->comment_date ) ) . '</a>';
			$html[] .= '</div>';
		$html[] .= '</div>';
		$html[] .= '<div class="ticket-response">';
			$html[] .= wpautop( $get_comment_by_id );
		$html[] .= '</div>';
...
/* AJAX action callback */
add_action( 'wp_ajax_delete_comment_candidacy', 'delete_comment_candidacy' );
add_action( 'wp_ajax_nopriv_delete_comment_candidacy', 'delete_comment_candidacy' );
function delete_comment_candidacy($post) {

	$postid = $_POST['object_id'];
	
	wp_delete_comment( $postid, true );
	
	wp_send_json( '<div style="margin-top: 15px; margin-bottom: 15px;">Le commentaire #' . $postid . ' a ete supprimé.</div>' );

	
}	

/* AJAX action callback */
add_action( 'wp_ajax_avis_employeur', 'avis_employeur' );
add_action( 'wp_ajax_nopriv_avis_employeur', 'avis_employeur' );
function avis_employeur($post) {

	$authorid = $_POST['object_id'];
	$avismessage = $_POST['avismessage'];
	...
	$html[] = '<div class="candidacy-response-cards-wrapper ' . $ramdonstring . '" style="padding-bottom: 25px;">';
		...
		$html[] .= '<br>';
		$html[] .= 'Horaire:';						
		$html[] .= get_user_meta( $post_id, 'horaire_key', true);
		$html[] .= ' - ';
		$html[] .= 'Superieur:';
		$html[] .= get_user_meta( $post_id, 'superieur_key', true);
		$html[] .= ' - ';
		$html[] .= 'Paie:';
		$html[] .= get_user_meta( $post_id, 'paie_key', true);
	$html[] .= '</div>';
	...
	if($get_avis){
		...
		foreach($get_avis as $avis){
 			...
 			if(get_user_meta( $avis->ID, 'authorid_key', true) == $authorid){
	...
	$array[] = 'Horaire:';						
	$array[] .= round($horaire_moyenne/$i, 2);
	$array[] .= ' - ';
	$array[] .= 'Superieur:';
	$array[] .= round($superieur_moyenne/$i, 2);
	$array[] .= ' - ';
	$array[] .= 'Paie:';
	$array[] .= round($paie_moyenne/$i, 2);
	
	$attr = array(implode($html), implode($array));
        ...
        $to = $author_email;
		$subject = sprintf ( __( 'Nouvelle avis #%s — %s — %s', 'monemploi' ), $post_id, get_the_title($post_id), get_bloginfo( 'name', 'display' ) );
		$headers = array('Content-Type: text/html; charset=UTF-8');
		
		$message[] .= '<p>';
		$message[] .= 'Nouvelle avis #' . $post_id;
		$message[] .= '</p>';
		$message[] .= '<a href="' . get_home_url() . '/employeur/?user='. $get_user_by_id->user_login .'">Voire lavis</a>';
...
/* AJAX action callback */
add_action( 'wp_ajax_delete_avis', 'delete_avis' );
add_action( 'wp_ajax_nopriv_delete_avis', 'delete_avis' );
function delete_avis($post) {

	$postid = $_POST['object_id'];
	$userid = $_POST['object_userid'];
	...
	if($get_avis){
		...
		foreach($get_avis as $avis){
			...
			if(get_user_meta( $avis->ID, 'authorid_key', true) == $userid){
	...
	$html[] = '<div style="padding-bottom: 15px;">';
		...
		$html[] .= '<div class="avis_error"></div>';
		
		$html[] .= '<label>Avis</label>';
		$html[] .= '<textarea id="avis-message-employeur" name="avis-message-employeur" class="avis-message-employeur" rows="5" cols="30"></textarea>';
		$html[] .= '<div class="number-of-char"></div>';
		
		$html[] .= '<label>Horaire</label>';
		$html[] .= '<input type="number" id="horaire-employeur" name="horaire-employeur" class="horaire-employeur" min="0" max="5" step=".01">';
		
		$html[] .= '<label>Supérieur</label>';
		$html[] .= '<input type="number" id="superieur-employeur" name="superieur-employeur" class="superieur-employeur" min="0" max="5" step=".01">';
		
		$html[] .= '<label>Paie</label>';
		$html[] .= '<input type="number" id="paie-employeur" name="paie-employeur" class="paie-employeur" min="0" max="5" step=".01">';
		
	$html[] .= '</div>';
	
	$html[] .= '<button class="avis-employeur-send" data-object-id="' .  $userid . '">';
		$html[] .= 'Soumettre';
	$html[] .= '</button>';

	
	wp_send_json( array('<div style="margin-top: 15px; margin-bottom: 15px;">Lavis #' . $postid . ' a ete supprimé.</div>', implode($array), implode($html)));

	
}

/* AJAX action callback */
add_action( 'wp_ajax_avis_employer', 'avis_employer' );
add_action( 'wp_ajax_nopriv_avis_employer', 'avis_employer' );
function avis_employer($post) {

	$authorid = $_POST['object_id'];
	$avismessage = $_POST['avismessage'];
	$ponctualite = $_POST['ponctualite'];
	$connaisance = $_POST['connaisance'];
	...
	$html[] = '<div class="candidacy-response-cards-wrapper ' . $ramdonstring . '" style="padding-bottom: 25px;">';
		...
		$html[] .= '<br>';
		$html[] .= 'Ponctualité:';						
		$html[] .= get_user_meta( $post_id, 'ponctualite_key', true);
		$html[] .= ' - ';
		$html[] .= 'Connaisance:';
		$html[] .= get_user_meta( $post_id, 'connaisance_key', true);
		$html[] .= ' - ';
		$html[] .= 'Attitude:';
		$html[] .= get_user_meta( $post_id, 'attitude_key', true);
	$html[] .= '</div>';
	...
	$array[] = 'Ponctualité:';						
	$array[] .= round($ponctualite_moyenne/$i, 2);
	$array[] .= ' - ';
	$array[] .= 'Connaisance:';
	$array[] .= round($connaisance_moyenne/$i, 2);
	$array[] .= ' - ';
	$array[] .= 'Attitude:';
	$array[] .= round($attitude_moyenne/$i, 2);
	...
	$message[] .= 'Nouvelle avis #' . $post_id;
	$message[] .= '</p>';
	$message[] .= '<a href="' . get_home_url() . '/employee/?user='. $get_user_by_id->user_login .'">Voire lavis</a>';
...
/* AJAX action callback */
add_action( 'wp_ajax_delete_avis_employer', 'delete_avis_employer' );
add_action( 'wp_ajax_nopriv_delete_avis_employer', 'delete_avis_employer' );
function delete_avis_employer($post) {
	...
	$html[] .= '<div style="padding-bottom: 15px;">';
		...
		$html[] .= '<div class="avis-error-employer"></div>';
		
		$html[] .= '<label>Avis</label>';
		$html[] .= '<textarea id="avis-message-employer" name="avis-message-employer" class="avis-message-employer" rows="5" cols="30"></textarea>';
		$html[] .= '<div class="number-of-char"></div>';
		
		$html[] .= '<label>Ponctualité</label>';
		$html[] .= '<input type="number" id="ponctualite-employer" name="ponctualite-employer" class="ponctualite-employer" min="0" max="5" step=".01">';
		
		$html[] .= '<label>Connaisance</label>';
		$html[] .= '<input type="number" id="connaisance-employer" name="connaisance-employer" class="connaisance-employer" min="0" max="5" step=".01">';
		
		$html[] .= '<label>Attitude</label>';
		$html[] .= '<input type="number" id="attitude-employer" name="attitude-employer" class="attitude-employer" min="0" max="5" step=".01">';
		
	$html[] .= '</div>';
	
	$html[] .= '<button class="avis-employer-send" data-object-id="' .  $userid . '">';
		$html[] .= 'Soumettre';
	$html[] .= '</button>';
