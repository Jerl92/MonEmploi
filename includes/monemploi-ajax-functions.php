...
function monemploi_ajax_scripts() {
	...
	wp_register_script( 'monemploi-ajax-employee-jobs-select-scripts', $url . "js/null.js", array( 'jquery' ), '1.0.0', true );
	wp_localize_script( 'monemploi-ajax-employee-jobs-select-scripts', 'horaire_jobs_select_monemploi_ajax_url', admin_url( 'admin-ajax.php', 'relative' ) );
	wp_enqueue_script( 'monemploi-ajax-employee-jobs-select-scripts' );
...
function chat_send($post) {
	...
	$html[] .= '<span style="font-weight: bold;">';
		$hide_seen = get_user_meta( $recive_userid, 'hide_seen_key', true);
...
function chat_offline_time($post) {
	...
	$hide_online = get_user_meta($recive_userid, 'hide_online_key', true);
	
	if($hide_online == 0 || $hide_online == '') {
		if($offline_time > 0) {
			$offline_calc = $current_time - $offline_time;
			$html = secondsToTime_ajax($offline_calc);
		} else {
			$offline_calc = 0;
			$html = secondsToTime_ajax($offline_calc);
		}
...
function chat_online_status($post) {
	...
	$userid = $_POST['userid'];
	    ...
	    $hide_online = get_user_meta($userid, 'hide_online_key', true);
...
/* AJAX action callback */
add_action( 'wp_ajax_jobs_horaire_select', 'jobs_horaire_select' );
add_action( 'wp_ajax_nopriv_jobs_horaire_select', 'jobs_horaire_select' );
function jobs_horaire_select($post) {

	$value = $_POST['value'];
	
	update_user_meta(get_current_user_id(), 'jobs_horaire_select', $value);
