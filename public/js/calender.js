        ...
        return false;
            ...
            function myCalendar(curr_month = "curr") {   
	              ...
	              var endweekunix = parseInt((new Date(endweekmonday).getTime() / 1000).toFixed(0));
                      ...
                      while (counter <= daysOfMonth[month]) {
                              ...
                              var datestring = calc_month+"/"+counter+"/"+year;
                              var timeunix = parseInt((new Date(datestring).getTime() / 1000).toFixed(0));
                              if(weekday_count === 8){
                                      tbody_html += "</tr>";
                                      weekday_count = 1;
                              }
			...
			if( timeunix >= startweekunix && timeunix < endweekunix) {
                            if (results.length == 1) {
                                    ...
                                    tbody_html += "<td class='"+td_class+" biweek' style='text-align: center; width: 14.28%;'><a href='"+current_url+"?daytime="+timeunix+"'>"+counter+"</a><br><span>"+results.length+" horaires</span></td>";
                            } else {
                                    tbody_html += "<td class='"+td_class+" biweek' style='text-align: center; width: 14.28%;'><a href='"+current_url+"?daytime="+timeunix+"'>"+counter+"</a></td>";
                            }
                       } else  {
                            if (results.length == 1) {
              ...
              var o = 0;
                      ...
                      jQuery('ul.gettimepay li').each(function(index, element) {
                        ...
                        while (counter <= daysOfMonth[month]) {
                              ...
                              var dayOfMonth = today.getDate();
                              var calc_month = parseInt(month) + 1;
                                if (/^\d$/.test(calc_month)) {
                                    var monthcalc_ = "0" + calc_month;
                                } else {
                                    var monthcalc_ = calc_month;
                                }
                                if (/^\d$/.test(counter)) {
                              ...
                              var datestring = monthcalc_+"/"+counter_+"/"+year;
                              var timeunix = parseInt((new Date(datestring).getTime() / 1000).toFixed(0));
                              if(weekday_count === 8){
                                      tbody_html += "</tr>";
                                          weekday_count = 1;
                ...
                jQuery.each(word, function(index, value) {
                            somofpayday.push([value[4], parseFloat(value[2])]);
                });
                ...
                var timeunixpayweek = parseInt((new Date(datestring_).getTime() / 1000).toFixed(0));
