...
function monemploi_new_employees_avis_widgets() {
	register_widget( 'monemploi_new_employees_avis_widgets');
}
add_action( 'widgets_init', 'monemploi_new_employees_avis_widgets' );

class monemploi_new_employees_avis_widgets extends WP_Widget {
	public function __construct() {
		// Instantiate the parent object
		parent::__construct(

			// Base ID of your widget
			'monemploi_new_employees_avis_widgets', 

			// Widget name will appear in UI
			__('monemploi_new_employees_avis_widgets', 'monemploi'), 
	...
	function widget( $args, $instance ) {
		...
		echo $args['before_title'] . $title . $args['after_title'];
		   ...
		   foreach ($users as $user) {
		            ...
		            $hide_widget_new_employees_avis = get_user_meta( $user->ID, 'hide_widget_new_employees_avis_key', true);
		            if($hide_widget_new_employees_avis == 0 || $hide_widget_new_employees_avis == '') {
		   		
		   		$hide_wdget_new_employees_avis_userids[$user->ID] = 0;
			    
			    } else {
		   		
		   		$hide_wdget_new_employees_avis_userids[$user->ID] = 1;
	    		...
	    		echo '<div>'; 
						...
						if($get_avis){
							...
							foreach($get_avis as $avis){
							    ...
							    if($avis_role == 'employer'){ 
							    
							    	foreach($hide_wdget_new_employees_avis_userids as $key => $value){ 
